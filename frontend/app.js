...
function actualizarResumen(productos) {
  ...
  const promedio = total
    ? productos.reduce((acc, p) => acc + Number(p.precio || 0), 0) / total
    : 0;

  if (totalProductos) totalProductos.textContent = total;
  if (stockAcumulado) stockAcumulado.textContent = stock;
  if (precioPromedio) precioPromedio.textContent = `$${promedio.toFixed(0)}`;
}
